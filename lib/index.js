...
const prefix = '.'
const botName = "Bills-BOT"
const ownerNumber = "5511952761206"
const ownerName = "Orochii"
const cdd = 480000 // 8 minutos
const ammOff = "off"
const blocked = []

//_LISTA DE CRS
const cr = "Bills-BOT v2.0"
...
tiringa.on('group-participants-update', async (anu) => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
            ...
            const totalchat = await Bills.chats.all()
			const botNumber = Bills.user.jid
			const ownerNumber = [`5511952761206@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`5511952761206@s.whatsapp.net`]
			...
			let pushname = Bills.contacts[sender] != undefined ? tiringa.contacts[sender].vname || Bills.contacts[sender].notify: undefined
           ...
           mess = {
				...
				only: {
					...
					admin: '❌SILÊNCIO MEMBRO COMUM VC Ñ TEM MORAL PRA USAR ESSE COMANDO❌' ,
...
⚠️AVISO:
      ...
      ❧ Talvez essa seja a ultima atualização 
...
case 'pão':
if (isOwner) return reply('Oi Orochii')
else {
